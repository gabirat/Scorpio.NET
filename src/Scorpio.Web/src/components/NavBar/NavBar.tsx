...
const NavBarMobile: React.SFC<NavBarMobileProps> = ({children, leftItems, onPusherClick, onToggle, rightItems, visible, changeNavBarCard, height}) => (
  <Sidebar.Pushable>
    <Sidebar
      as={Menu}
      animation="overlay"
      ...
      visible={visible}
    >
      {leftItems.map((item: NavBarItem) => <Menu.Item {...item} onClick={() => changeNavBarCard(item.url)}/>)}
      {rightItems.map((item: NavBarItem) => <Menu.Item {...item} onClick={() => changeNavBarCard(item.url)}/>)}
    ...
    >
      ...
      <Menu fixed="top" inverted style={{height: height ? height : defaultHeight}}>
        ...
        <Menu.Menu position="right">
          {rightItems.map((item: NavBarItem) => <Menu.Item
            onClick={() => changeNavBarCard(item.url)} {...item}/>)}
        </Menu.Menu>
      </Menu>
...
const NavBarDesktop: React.SFC<NavBarDesktopProps> = ({leftItems, rightItems, changeNavBarCard, height, children}) => (
  <>
    <Menu fixed="top" inverted style={{height: height ? height : defaultHeight}}>
      ...
      {leftItems.map((item: NavBarItem) => <Menu.Item onClick={(e) => changeNavBarCard(item.url)} {...item}/>)}
      <Menu.Menu position="right">
        {rightItems.map((item: NavBarItem) => <Menu.Item
          onClick={(e) => changeNavBarCard(item.url)} {...item}/>)}
      </Menu.Menu>
    </Menu>
  ...
  render() {
    ...
    return <>
      ...
      <Responsive minWidth={Responsive.onlyTablet.minWidth}>
        <NavBarDesktop leftItems={leftItems} rightItems={rightItems} height={height}
                       changeNavBarCard={this.changeNavBarCard}>
          {children}
        </NavBarDesktop>
      </Responsive>
